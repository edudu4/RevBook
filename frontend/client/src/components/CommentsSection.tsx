...
import { MessageSquare, Trash2, Edit2, Check, X, Smile, Reply } from 'lucide-react';
...
export default function CommentsSection({ resenhaId, onComentarioAdicionado }: CommentsSectionProps) {
  ...
  const handleResponderComentario = async (parentId: number) => {
  ...
  const comentariosPrincipais = comentarios.filter((c) => !c.parentId);
  const respostasPorPai = new Map<number, Comentario[]>();
  comentarios.forEach((c) => {
    if (c.parentId) {
      const lista = respostasPorPai.get(c.parentId) ?? [];
      lista.push(c);
      respostasPorPai.set(c.parentId, lista);
    }
  });

  const renderCorpoComentario = (comentario: Comentario) => (
    <>
      ...
      <div className="flex items-center gap-2 flex-wrap mt-3 pt-3 border-t border-border">
        ...
        {isAuthenticated && (
          <div className="relative">
            <button
              onClick={() =>
                setActiveReactionPicker(activeReactionPicker === comentario.id ? null : comentario.id)
              }
        ...
        {!comentario.parentId && (
          <button
            onClick={() => {
              ...
              setRespondingId(respondingId === comentario.id ? null : comentario.id);
              setReplyContent('');
            }}
            className="flex items-center gap-1 px-2 py-1 rounded-full text-sm bg-muted text-foreground hover:bg-muted/80 transition-colors"
          >
      ...
      {respondingId === comentario.id && (
        <div className="mt-3 pl-4 border-l-2 border-border">
          <textarea
            value={replyContent}
            onChange={(e) => setReplyContent(e.target.value)}
            placeholder={`Responder a ${comentario.nomeUsuario}...`}
            rows={2}
            className="w-full px-3 py-2 border border-border rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-accent mb-2"
          />
          <div className="flex gap-2">
            <Button
              onClick={() => handleResponderComentario(comentario.id)}
              disabled={enviandoResposta || !replyContent.trim()}
              size="sm"
          ...
          comentariosPrincipais.map((comentario) => (
            <div key={comentario.id}>
              <div className="p-4 bg-card border border-border rounded-lg">
                {renderCorpoComentario(comentario)}
              </div>

              {(respostasPorPai.get(comentario.id) ?? [])
                .slice()
                .reverse()
                .map((resposta) => (
                  <div key={resposta.id} className="ml-6 sm:ml-10 mt-3 p-4 bg-card border border-border rounded-lg">
                    {renderCorpoComentario(resposta)}
                  </div>
                ))}
            </div>
          ))
